...
controller.A.onEvent(ControllerButtonEvent.Pressed, function () {
    if ( player.image == gunLeft || player.image == gunShootLeft){
        sprites.createProjectileFromSprite(bulletLeft, player, -900, 0)
    }
    else{
        sprites.createProjectileFromSprite(bulletRight, player, 900, 0)
    }
})
...
function zombieSpawn(zombies: any[]){
    for(let i = 0; i < zombies.length; i++){
        ...
        for (let j = 0; j < waveNumber; j++){
           tiles.placeOnRandomTile( sprites.create(tempImg , SpriteKind.Enemy),  sprites.dungeon.collectibleRedCrystal)
        }
    }
}
if (zombieCounter == 0) {
...
let player = sprites.create(gunLeft, SpriteKind.Player)
